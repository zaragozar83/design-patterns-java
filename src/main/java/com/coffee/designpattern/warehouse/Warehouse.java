...
import com.coffee.designpattern.warehouse.util.CsvReader;
...
public final class Warehouse {
    ...
    private void readOrders() throws FileNotFoundException, WarehouseException {
        ...
        while (reader.hasNextRow()) {
            ...
            int customerId;
            try {
                customerId = Integer.parseInt(row.get(1));
            } catch (NumberFormatException ex) {
            ...
            boolean pending = Boolean.parseBoolean(row.get(3));
            ...
            for (String orderDetail : row.subList(4, row.size())) {
                ...
                int productId;
                try {
                    productId = Integer.parseInt(productIdAndQuantity[0]);
                } catch (NumberFormatException ex) {
                ...
                int quantity;
                try {
                    quantity = Integer.parseInt(productIdAndQuantity[1]);
                } catch (NumberFormatException ex) {
